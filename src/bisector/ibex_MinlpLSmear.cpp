...
MinlpLSmear::~MinlpLSmear() {
	delete mylinearsolver;
}


  LPSolver::Status MinlpLSmear::getdual(IntervalMatrix& J, const IntervalVector& box, Vector& dual) const {
...
int MinlpLSmear::var_to_bisect(IntervalMatrix& J, const Cell& cell) const {
  ...
  int lvar = -1;
	...
	if (lsmode==MINLPLSMEAR_MG) { //compute the Jacobian in the midpoint
	...
	if (stat == LPSolver::Status::Optimal) {
		...
		for (int j=0; j<sys.nb_var; j++) {
			...
			if ((j!= goal_var() && b[j]) && (!too_small(box,j)) && (box[j].mag() <1 ||  box[j].diam()/ box[j].mag() >= prec(j))){
